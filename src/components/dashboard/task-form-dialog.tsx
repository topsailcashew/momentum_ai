...
import { PlusCircle, Calendar as CalendarIcon, Trash2, ChevronDown, Loader2 } from 'lucide-react';
...
export function TaskFormDialog({ task, categories, projects, open: externalOpen, onOpenChange: externalOnOpenChange, onSave, onDelete, isPending, children, defaultDeadline }: TaskFormDialogProps) {
  ...
  return (
    <ResponsiveModal open={open} onOpenChange={onOpenChange}>
      {triggerButton && <ResponsiveModalTrigger asChild>{triggerButton}</ResponsiveModalTrigger>}
      <ResponsiveModalContent className="sm:max-w-lg max-h-[90vh] flex flex-col p-0 gap-0">
        <ResponsiveModalHeader className="p-6 pb-2 flex-none">
          <ResponsiveModalTitle>{isEditing ? 'Edit Task' : 'Add a new task'}</ResponsiveModalTitle>
          <ResponsiveModalDescription>
        ...
        <ResponsiveModalFooter className={cn(
          ...
          {isEditing && onDelete && (
            <AlertDialog>
              <AlertDialogTrigger asChild>
                <Button type="button" variant="destructive" size="sm" className="w-full sm:w-auto mb-2 sm:mb-0">
          ...
          <div className="flex gap-2 w-full sm:w-auto">
            ...
            <Button type="submit" form="task-form" disabled={isPending} className="flex-1 sm:flex-initial">
